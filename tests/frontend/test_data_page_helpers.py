...
def test_fill_table_input_found(helpers_and_db):
    ...
    assert any(url.startswith("https://www.margonem.pl/profile/view,5111553#char_") for url in urls)
...
def test_get_unique_chars_by_profile(helpers_and_db, test_rows):
    ...
    for item in results:
        assert item['profile'].startswith("https://www.margonem.pl/profile/view,5111553#char_")
